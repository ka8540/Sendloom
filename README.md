...
The normal people pipeline is database-only: a durable exact shared-cache fingerprint, the durable same-company shared pool, and the current user's already-materialized company people are checked in that order. Role and location guards are applied to every reuse path. Stored people do not expire; an ordinary miss returns no results and never calls Apify or tops a partial result up to 10. **Add 10 more** is the only user action that may call Apify: it consumes unused matching database candidates first, then continues from the saved provider page (or page 1 when no continuation exists), with persisted `ProspectSearchPerson` grants as the source of truth for counts and deduplication.

Discover company identity is domain-first everywhere: normalized official domain → trusted LinkedIn company slug when no official domain exists → normalized company name when neither exists. Historical `linkedin:*` shared-cache rows remain reusable without a backfill when their stored trusted domain matches the newly resolved official domain. For same-company reuse, an exact match between the cache entry's normalized requested locations and the new request may supply provider-search provenance when an individual cached person has missing or incomplete geography; explicit country contradictions remain rejected, and provenance is never carried across different requests such as United States → San Francisco.
