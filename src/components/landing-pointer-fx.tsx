...
/**
 * Pointer-driven atmosphere for the landing page:
 * - a soft cursor-following light behind the content
 * - tilt + spotlight CSS variables on `[data-card-fx]` cards
 * - gentle pointer parallax on `[data-pointer-parallax]` elements
 *
 * Transform/opacity only. The shared rAF loop runs while values are settling
 * and stops when the scene is at rest, so an idle pointer costs nothing.
 * Disabled entirely for coarse pointers and `prefers-reduced-motion: reduce`.
 */
export function LandingPointerFX() {
  ...
  useEffect(() => {
    ...
    const parallaxItems = Array.from(document.querySelectorAll<HTMLElement>("[data-pointer-parallax]")).map(
      (element) => ({
        element,
        depth: Number(element.dataset.pointerParallax) || 12
      })
    );
    ...
    const step = () => {
      ...
      if (glow) {
        glow.style.transform = `translate3d(${(glowX - 290).toFixed(1)}px, ${(glowY - 290).toFixed(1)}px, 0)`;
        if (!glowVisible) {
    ...
    const cardCleanups = cards.map((card) => {
      ...
      const onMove = (event: PointerEvent) => {
        ...
        cardFrame = window.requestAnimationFrame(() => {
          ...
          card.style.setProperty("--tilt-x", `${((y / rect.height - 0.5) * -3.4).toFixed(2)}deg`);
          card.style.setProperty("--tilt-y", `${((x / rect.width - 0.5) * 4.2).toFixed(2)}deg`);
