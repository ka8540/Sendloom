...
function asUrl(raw: string): URL | null {
  try {
    ...
    try {
      return new URL(raw, "https://www.google.com");
    } catch {
      return null;
    }
  }
...
export function inspectGoogleResultRedirect(raw: unknown): {
  ...
  if (!url || !/^(?:www\.)?google\.com$/i.test(url.hostname) || !GOOGLE_REDIRECT_PATHS.has(url.pathname.toLowerCase())) {
    return null;
  }
  for (const param of GOOGLE_DESTINATION_PARAMS) {
    ...
    return /^https?:\/\//i.test(destination)
      ? { wrapperUrl: url, destination, opaque: false }
      : { wrapperUrl: url, destination: null, opaque: true };
  }
...
function identityFromUrl(url: URL): LinkedInProfileUrlResolution {
  ...
  let slug: string;
  try {
    slug = decodeURIComponent(match[1]).normalize("NFC").toLowerCase();
  } catch {
    return { ok: false, reason: "invalidUrl" };
  }
...
/** Strict public-person URL identity. It never fetches the LinkedIn profile. */
