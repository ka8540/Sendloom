...
const envSchema = z
  .object({
    ...
    WEB_SEARCH_PROVIDER: z.preprocess(
      ...
      z.enum(["none", "serper", "brave", "you", "playwright_google", "brightdata_google"]).default("none")
    ),
    BRIGHTDATA_API_KEY: z.preprocess(value => value === "" ? undefined : value, z.string().trim().min(1).optional()),
    BRIGHTDATA_SERP_ZONE: z.preprocess(value => value === "" ? undefined : value, z.string().trim().min(1).optional()),
    DISCOVER_PUBLIC_MAX_PAGES: z.coerce.number().int().min(1).max(10).default(10),
    DISCOVER_PUBLIC_MAX_RESULTS: z.coerce.number().int().min(1).max(100).default(100),
    SERPER_API_KEY: z.string().min(1).optional(),
    BRAVE_SEARCH_API_KEY: z.string().min(1).optional(),
    PLAYWRIGHT_GOOGLE_MAX_PAGES: z.coerce.number().int().min(1).max(20).default(10),
    PLAYWRIGHT_GOOGLE_MAX_CANDIDATES: z.coerce.number().int().min(1).max(500).default(250),
    PLAYWRIGHT_GOOGLE_NAVIGATION_TIMEOUT_MS: z.coerce.number().int().min(1000).max(20000).default(10000),
...
function readRawEnv() {
  return {
    ...
    DISCOVER_PEOPLE_PROVIDER: process.env.DISCOVER_PEOPLE_PROVIDER,
    WEB_SEARCH_PROVIDER: process.env.WEB_SEARCH_PROVIDER,
    BRIGHTDATA_API_KEY: process.env.BRIGHTDATA_API_KEY,
    BRIGHTDATA_SERP_ZONE: process.env.BRIGHTDATA_SERP_ZONE,
    DISCOVER_PUBLIC_MAX_PAGES: process.env.DISCOVER_PUBLIC_MAX_PAGES,
    DISCOVER_PUBLIC_MAX_RESULTS: process.env.DISCOVER_PUBLIC_MAX_RESULTS,
    SERPER_API_KEY: process.env.SERPER_API_KEY,
    BRAVE_SEARCH_API_KEY: process.env.BRAVE_SEARCH_API_KEY,
    YDC_API_KEY: process.env.YDC_API_KEY,
    PLAYWRIGHT_GOOGLE_MAX_PAGES: process.env.PLAYWRIGHT_GOOGLE_MAX_PAGES,
    PLAYWRIGHT_GOOGLE_MAX_CANDIDATES: process.env.PLAYWRIGHT_GOOGLE_MAX_CANDIDATES,
    PLAYWRIGHT_GOOGLE_NAVIGATION_TIMEOUT_MS: process.env.PLAYWRIGHT_GOOGLE_NAVIGATION_TIMEOUT_MS,
